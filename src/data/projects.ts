...
export const projects: Project[] = [
  ...
  },
  {
    ...
    liveUrl: null,
    features: [
      "3 role-based consoles (Owner / Collector / Operator) with middleware route protection & server-side guards",
      "Kanban phase board with dynamic columns from configurable phase templates and phase undo",
      "Transaction-safe sequential batch & invoice numbers (UNIQUE indexes + row locks with race-condition retry)",
      "Camera barcode/QR scanning (ZXing) and printable QR batch labels",
      "End-to-end batch traceability — fabric intake → production phases → finished goods → every sale",
      "POS with sales, returns & exchanges, due collection, dues aging report, and printable invoices & money receipts",
      "Owner dashboard with KPIs, live polling (SWR), Recharts analytics, and configurable low-stock/bottleneck alerts",
      "Offline mode — shop-floor mutations queue locally and auto-replay on reconnect",
      "Security: bcrypt + JWT sessions (NextAuth v5), login lockout, session versioning, full audit trail with before/after data",
      "Bilingual UI (English / Bangla), 23 screens, 48 API endpoints, Vitest tests + GitHub Actions CI",
    ],
